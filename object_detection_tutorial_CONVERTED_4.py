...
TEST_IMAGE_PATHS = [ os.path.join(PATH_TO_TEST_IMAGES_DIR, 'image{}.jpg'.format(i)) for i in range(40, 41) ]
...
# 중심좌표와 박스좌표를 알기위한 빈 list 선언
  ...
  for i in range(int(r)):    # 선택한 피사체 갯수만큼 반복분 돌린다
      ...
      tmp_center[1] = tmp_center[1] + float(center_dic[k][1])    # 피사체들의 중심좌표중 y좌료를 더해준다.
...
# frame_offset_funtion(full_width, full_height, tar_point[0], tar_point[1], frame_xmin, frame_xmax, frame_ymin, frame_ymax)
def frame_offset_funtion(width, height, x, y, a, b, c, d, q, w, e, r):
    ...
    if frame_xmin < 0 :
        ...
        overpadding_xmin = -1.0 * frame_xmin
        frame_xmin = frame_xmin + overpadding_xmin
        frame_xmax = frame_xmax + overpadding_xmin
    if frame_ymin < 0 :
